...
const MyContribution = () => {
    ...
    const { isPending, isError, error, refetch, data: Data } = useQuery({
        queryKey: ['Data'],
        queryFn: async () => {
            const res = await fetch(`http://localhost:5000/user/all-contribution/${user?.email}`);
            return res.json();
        }
    })
    ...
    return (
        <div className='container mx-auto search-page py-10'>
            <Helmet>
                <title>scholars Repository || My Contribution</title>
            </Helmet>
                    ...
                    <tbody className=''>
                        {
                            Data?.map((data, index) =>
                                ...
                                >
                                    ...
                                    <button className="btn hover:bg-teal-700 text-white lg:text-lg bg-[#2F5597] md:w-1/2" onClick={()=>handleDownload(data.webContentLink)}><FaCloudDownloadAlt/> Download</button> </td>
