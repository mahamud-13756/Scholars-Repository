...
const RecentBooks = () => {
    ...
    const { isPending, isError, error, refetch, data: recentBooksData } = useQuery({
        queryKey: ['recentBooks'],
        queryFn: async () => {
            const res = await fetch('http://localhost:5000/recent-books');
            return res.json();
        }
    });

    if(recentBooksData){
        console.log("consultant:", recentBooksData);
    }
...
const formatTimeDifference = (createdTime) => {
    ...
    return (
        <div className="w-full my-10">
            <SectionTitle heading={"Recently Shared Books"}></SectionTitle>
            ...
            <div className="w-full ">
                <div className=" container mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2 my-1 px-5 md:px-0">
                    {
                        recentBooksData?.map((data, index) =>
