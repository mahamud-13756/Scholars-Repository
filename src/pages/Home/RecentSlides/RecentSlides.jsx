...
const RecentSlides = () => {
    ...
    const { isPending, isError, error, refetch, data: recentSlidesData } = useQuery({
        queryKey: ['recentSlides'],
        queryFn: async () => {
            const res = await fetch('http://localhost:5000/recent-slides');
            return res.json();
        }
    });

    if(recentSlidesData){
        console.log("consultant:", recentSlidesData);
    }
...
const formatTimeDifference = (createdTime) => {
    ...
    return (
        <div className="w-full my-10">
            <SectionTitle heading={"Recently Shared Slides"}></SectionTitle>
            ...
            <div className="w-full ">
                <div className=" container mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2 my-1 px-5 md:px-0">
                    {
                        recentSlidesData?.map((data, index) =>
